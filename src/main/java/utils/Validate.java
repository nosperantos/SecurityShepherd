...
public class Validate {
  ...
  public static Cookie getToken(Cookie[] userCookies) {
    ...
    if (theToken != null) {
      ...
      // The Token is currently designed to be a random Big Integer. If the Big Integer Case does
      // not work, the token has been modified. Potentially in a malicious manner
      try {
  ...
  public static String validateEncryptionKey(String userSalt) {
    ...
    if (keySize == 16) {
      ...
      } else // Shorter than 16
      {
        ...
        for (int i = 0; i < howManyTimes; i++) newKey += userSalt;
  ...
  public static String validateLanguage(HttpSession ses) {
    ...
    try {
      ...
      if (lang != null || !lang.isEmpty()) {
        if (lang.matches(".[a-z]{2}-[A-Z]{2}$")) result = lang;
      }
      ...
      if (ses.getAttribute("logout") != null) {
        ...
        if (ses.getAttribute("userRole") != null) {
          try {
            ...
            if (!result) log.fatal("User Role Parameter Tampered. Role = " + role);
            else {
  ...
  public static boolean validateTokens(Cookie cookieToken, Object requestToken) {
    ...
    if (!cookieNull && !requestNull) {
      try {
        ...
        if (!cookieEmpty && !requestEmpty) result = theRequest.compareTo(theCookie) == 0;
        else if (cookieEmpty) log.error("Cookie Token Empty");
        else if (requestEmpty) log.error("Request Token Empty");

        if (!result) log.error("CSRF Tokens did not match");
      } catch (Exception e) {
        log.error("CSRF in Request Error: " + e.toString());
      }
    } else {
      if (cookieNull) log.error("Cookie Token was Null");
      else if (requestNull) log.error("Request Token was Null");
    }
    return result;
  }
  ...
  */
  ...
  public static boolean validHostUrl(String hostUrl) {
    ...
    if (!result) log.error("URL Doesn't end with a forward slash. Very likely wrong");
    return result;
  }
