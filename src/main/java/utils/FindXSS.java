...
public class FindXSS {
  ...
  public static boolean findCsrf(String messageForAdmin, String falseId) {
    // Find a HTML tag
    while (messageForAdmin.contains("< ")) messageForAdmin = messageForAdmin.replaceAll("< ", "<");
    while (messageForAdmin.contains(" >")) messageForAdmin = messageForAdmin.replaceAll(" >", ">");
      ...
      if (tempEnd == -1) {
        ...
        if (tempMessage.contains(" src")) {
          ...
          if (tempMessage.startsWith("\"")) {
            ...
            if (endOfUrl == -1) endOfUrl = tempMessage.length();
            else endOfUrl--;
  ...
  public static boolean findCsrfAttackUrl(String theUrl, String csrfAttackPath) {
    ...
    try {
      ...
      if (!validAttack) log.debug("Invalid Solution: Bad Path or Above");
    } catch (MalformedURLException e) {
