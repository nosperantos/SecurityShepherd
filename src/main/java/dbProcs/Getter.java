...
public class Getter {
      ...
      callstmt =
          conn.prepareCall(
              "SELECT userId, userName, userPass, userRole, badLoginCount, tempPassword, classId,"
                  + " suspendedUntil FROM `users` WHERE userName = ?");
    ...
    if (userFound) {
      ...
      if (userVerified) {
        ...
        if (!result[1].equalsIgnoreCase(
            userName)) // If somehow this functionality has been compromised to sign in as
  ...
  public static String getCsrfForumWithIframe(
    ...
    try {
      if (classId != null) {
        ...
        if (counter > 0) log.debug("Added a " + counter + " row table");
        else log.debug("No results from query");
        // Table end
        htmlOutput += "</table>";
      } else {
    ...
    try {
      ...
      while (resultSet.next()) // For each user in a class
      {
        ...
        if (resultSet.getString(1) != null) {
          ...
          if (goldMedals > 0) goldDisplayStyle = displayMedal;
          if (silverMedals > 0) silverDisplayStyle = displayMedal;
          if (bronzeMedals > 0) bronzeDisplayStyle = displayMedal;
          ...
          if (goldMedals > 0 || silverMedals > 0 || bronzeMedals > 0) {
            ...
            if (goldMedals > 0) userMedalString += goldMedals + " gold";
            if (silverMedals > 0) {
            ...
            if (goldMedals + silverMedals + bronzeMedals > 1) userMedalString += "s";
          }
          ...
          jsonInner.put(
              ...
              new String(
                  Encode.forHtml(resultSet.getString(2))
                      ...
                      + score
                      + " points"
                      + userMedalString)); // User name encoded for title attribute
      ...
      if (resultAmount > 0) result = json.toString();
  ...
  public static boolean getModuleKeyType(String ApplicationRoot, String moduleId) {
    ...
    try {
      ...
      if (theKeyType) log.debug("Module has hard coded Key");
      else log.debug("Module has user specific Key");
    } catch (Exception e) {
      log.error("Module did not exist: " + e.toString());
      theKeyType = true;
    }
        ...
        if (resultSet.getString(1) != null) {
          result +=
              "<tr><td>"
                  ...
                  + resultSet.getInt(2);
          if (resultSet.getInt(2) > 6) result += " Modules";
    ...
    try {
      ...
      while (resultSet.next()) // For each user in a class
      {
        ...
        if (resultSet.getString(1) != null) {
          ...
          jsonInner.put(
              "progressBar",
              new Integer(resultSet.getInt(2) * widthOfUnitBar)); // Progress Bar Width
  ...
  private static int getTounnamentSectionFromRankNumber(int rankNumber) {
    if (rankNumber < fieldTrainingCap) return 1;
    else if (rankNumber < privateCap) return 2;
    else if (rankNumber < corporalCap) return 3;
    else if (rankNumber < sergeantCap) return 4;
    else if (rankNumber < lieutenantCap) return 5;
    else if (rankNumber < majorCap) return 6;
    else if (rankNumber < admiralCap) return 7;
    else return 7; // Max level is 7.
  }
  /**
    ...
    try {
      ...
      while (levels.next()) {
        ...
        if (getTounnamentSectionFromRankNumber(levels.getInt(5)) > currentSection) {
          // This level is not in the same level band as the previous level. So a new Level Band
          // Header is required on the master list before we add the entry.
      ...
      if (levelMasterList.isEmpty()) {
        ...
        // List is complete, but we need to close the last list we made, which deinfetly exists as
        // the levelmasterList is not empty
        levelMasterList += "</ul>";
    ...
    try {
      ...
      while (levels.next()) {
        ...
        if (ModulePlan.isIncrementalFloor()) {
          ...
          if (moduleCompleted
              || (!moduleCompleted
                  && thisModuleIsOpen)) // If its completed or if this is the first not completed
          {
            moduleOpen = true;
  ...
  public static boolean isCsrfLevelComplete(
    ...
    try {
      ...
      if (result) log.debug("CSRF Level is complete");
    } catch (SQLException e) {
