      ...
      if (Validate.validateTokens(tokenCookie, tokenParmeter) && canSeeScoreboard) {
        ...
        if (ScoreboardStatus.getClassSpecificScoreboard()) {
          if (ses.getAttribute("userRole").toString().compareTo("admin") == 0) {
            ...
            else scoreboardClass = new String();
          }
        } else {
        ...
        if (jsonOutput.isEmpty()) jsonOutput = "ERROR: No Scoreboard Data Right Now";
        out.write(jsonOutput);
      } else {
        if (!canSeeScoreboard) out.write("ERROR: Scoreboard is not currently available");
        else out.write("ERROR: Please refresh page!");
      }
    } else {
