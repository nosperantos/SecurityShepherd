...
public class SessionManagement7SecretQuestion extends HttpServlet {
  ...
  private static org.apache.log4j.Logger log =
      Logger.getLogger(SessionManagement7SecretQuestion.class);
        ...
        if (validAnswer(subAns)) {
          ...
          try {
            if (Validate.isValidEmailAddress(subEmail) && subAns.length() > 5) {
              ...
              if (subAns.length() < 5) htmlOutput += bundle.getString("question.invalidAns");
              else htmlOutput += bundle.getString("question.invalidEmail");
            }
  ...
  private static boolean validAnswer(String submittedAns) {
    for (int i = 0; i < possibleAnswers.length; i++) {
      if (possibleAnswers[i].equalsIgnoreCase(submittedAns)) return true;
    }
    return false;
  }
}
