          ...
          if (!module.get("moduleId").toString().equalsIgnoreCase(moduleId)) {
            ...
            if (module.getBoolean("moduleOpen")) {
              ...
              String message =
                  "Module "
                      + module.get("moduleId")
                      + " is marked as completed when no module should be marked as complete for"
                      + " this user";
              log.debug(message);
              fail(message);
            } else {
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (!theResponse.getJSONObject(0).get("levelMode").toString().equalsIgnoreCase("ctf")) {
          ...
          if (!module.get("moduleId").toString().equalsIgnoreCase(moduleId)) {
            ...
            log.debug(message);
            fail(message);
          } else if (!(module.getInt("scoredPoints")
              == 15)) // Max points for first to complete first challenge
          {
            String message =
