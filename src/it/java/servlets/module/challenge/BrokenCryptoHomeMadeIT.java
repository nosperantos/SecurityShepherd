      ...
      if (csrfToken.isEmpty()) {
        ...
        String solutionString =
            BrokenCryptoHomeMade.generateUserSolutionKeyOnly(
                ...
                userName); // Just Create the Key From Source And then Send it to the servlet
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (servletResponse.contains(bundle.getString("result.failed"))) {
          ...
          fail(message);
            ...
            bundle.getString("insecureCyrptoStorage.homemade.badanswer.warning"))) {
    ...
    try {
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (servletResponse.contains(bundle.getString("result.failed"))) {
          ...
          String message =
              new String(
                  "Response contained a 'You are not locked out yet' message when we should be"
                      + " locked out");
          log.fatal(message);
          fail(message);
        } else if (!servletResponse.contains(
    ...
    try {
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (!servletResponse.isEmpty()) {
          String message =
              new String("Expected Blank Response for Bad CSRF Token. Got Data Instead");
          log.fatal(message);
          fail(message);
        }
      }
  ...
  public void testLevelCustomName() {
    ...
    try {
      ...
      if (csrfToken.isEmpty()) {
        ...
        String solutionString =
            BrokenCryptoHomeMade.generateUserSolutionKeyOnly(
                BrokenCryptoHomeMade.challenges.get(0).get(1),
                name); // Just Create the That should be in table and search for that
      ...
      if (csrfToken.isEmpty()) {
        ...
        if (servletResponse.contains(bundle.getString("result.failed"))) {
          ...
          fail(message);
            ...
            bundle.getString("insecureCyrptoStorage.homemade.nameTooShort"))) {
