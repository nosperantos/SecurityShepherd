...
public class GetterTest {
  ...
  public static boolean verifyTestAdmin(String applicationRoot, String userName, String password)
    ...
    try {
      ...
      if (user == null || user[0].isEmpty()) {
        log.debug(
            "Test Failed. User not found in DB. Adding user to DB and Retesting before reporting"
                + " failure");
        Setter.userCreate(
      ...
      if (user != null && !user[0].isEmpty()) {
        ...
        log.error(
            "Couldnt verify that " + userName + " could authenticate at all. Throwing Exception");
  ...
  public static boolean verifyTestUser(
      String applicationRoot, String userName, String password, String theClass) throws Exception {
    boolean result = false;
    try {
      ...
      if (className.isEmpty()) {
        ...
        if (user != null && !user[0].isEmpty()) {
          ...
          if ((user[4] == null || user[4].isEmpty()) && !theClass.isEmpty()) {
            ...
            log.debug("Need to update user's class");
            Setter.updatePlayerClass(applicationRoot, theClass, user[0]);
          } else log.debug("User in class submitted already");
          result = true;
        } else {
          log.error(
              "Couldnt verify that " + userName + " could authenticate at all. Throwing Exception");
  ...
  public void testCheckPlayerResultWhenModuleComplete() {
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        } else fail("Could not Open All Modules");
      } else {
      ...
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          ...
          if (incrementalModules.indexOf("You've Finished!")
              > -1) // IF no modules are open, this is the expected leading string
          {
  ...
  public void testGetJsonScoreClassSpecific() {
    ...
    String insecureDirectObjectRefLesson =
        "0dbea4cb5811fff0527184f99bd5034ca9286f11"; // Direct Object Reference Module
    try {
      try {
        classId = findCreateClassId(className);
          ...
          if (markLevelCompleteTest != null)
            markLevelCompleteTest =
                Setter.updatePlayerResult(
                    ...
                    1,
                    1);
          else fail("Could Not Mark Level as complete by User 1");
          if (markLevelCompleteTest != null) {
            boolean pass = false;
      ...
      if (verifyTestUser(applicationRoot, userName, userName, classId)
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (markLevelCompleteTest != null) {
            ...
            for (int i = 0; i < scoreboardJson.size(); i++) {
              JSONObject scoreRowJson = (JSONObject) scoreboardJson.get(i);
              if (scoreRowJson.get("username").toString().compareTo(userName)
                  == 0) // Therefore not encoded for HTML
              {
                fail("Found " + userName + " in scoreboard");
              }
      ...
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!Getter.getModuleAddress(applicationRoot, inscureDirectObjectLesson, userId)
            ...
            if (markLevelCompleteTest != null) {
                  ...
                  > -1) // Menu Should include this at least once
              {
                if (lessonsMenu.indexOf(inscureDirectObjectLesson)
                    > -1) // This module should be in the response
                {
                  if (lessonsMenu.indexOf(
                          "<img src='css/images/completed.png'/><a class='lesson' id='"
                              + inscureDirectObjectLesson)
                      > -1) // This module should be returned as completed
                  {
                    if (lessonsMenu.indexOf(
            ...
            } else fail("Could not mark module as complete");
          } else {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
              ...
              == -1) // Menu Should not include this when modules closed
          {
            if (lessonsMenu.indexOf("No lessons found")
                > -1) // English string should exist in output based on the submitted locale
            {
              log.debug(
      ...
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.openAllModules(applicationRoot, false)
          ...
          if (!Getter.getModuleAddress(applicationRoot, insecureCryptoLesson, userId).isEmpty()) {
            ...
            return;
          } else fail("Could not Insecure Crypto Lesson as Opened by user");
        } else fail("Could not Open All Modules");
      } else {
        fail("Could not verify user (No Exception Failure)");
      }
    ...
    try {
      if (verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (Setter.closeAllModules(applicationRoot)) {
          ...
          } else fail("Could Get Module Address when marked as closed");
        } else fail("Could not Close All Modules");
      } else {
        fail("Could not verify user (No Exception Failure)");
      }
    } catch (Exception e) {
    ...
    if (Setter.openAllModules(applicationRoot, false)
      ...
      if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
        ...
        fail("No Close Meny Detected in Output");
          ...
          < 0) // Should be empty as all modules should be open
      {
        log.fatal(
    ...
    if (Setter.closeAllModules(applicationRoot)) {
      ...
      if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
        ...
        fail("No Close Meny Detected in Output");
          ...
          < 0) // Should be empty as all modules should be closed
      {
        log.fatal(
    ...
    if (Setter.openOnlyMobileCategories(applicationRoot)) {
      ...
      if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
        ...
        fail("No Close Meny Detected in Output");
          ...
          > 0) // Should not be empty as Web Levels should be closed
      {
        log.fatal(
        ...
        fail("Modules are in the 'toClose' list when web modules should already be closed");
          ...
          > 0) // Should not be empty as Mobile Levels should be open
      {
        log.fatal(
        ...
        if (toCloseList.indexOf(dataStorageLessonId) == -1) {
          ...
          // Verify the correct number of modules are open/closed (At this point the Menu is fine.
          // This is really now testing the mobile/web setter filters)
          ...
          if (mobileDbModuleCount != numberOfMobileLevelsOpen) {
            fail(
                "There are "
                    ...
                    + mobileDbModuleCount
                    + " in the DB");
            // This may mean that the where clause in the setter method is not correct and needs to
            // be updated
  ...
  public void testGetPlayersByClass() {
    try {
      ...
      try {
        ...
        if (i != 9) {
          if (i < 9) fail("Too Few Users Returned");
          else if (i > 9) fail("Too Many Users Returned");
          else {
  ...
  public void testGetAdmins() {
    try {
      ...
      try {
        ...
        if (i != 10) {
          if (i < 10) fail("Too Few Admins Returned");
          else if (i > 10) fail("Too Many Admins Returned");
          else {
