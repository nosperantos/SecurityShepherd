...
public class SetterTest {
        ...
        if (!Setter.openAllModules(applicationRoot, false)
          ...
          if (Getter.getModuleAddress(applicationRoot, moduleId, userId).isEmpty()) {
            ...
            if (markLevelCompleteTest != null) {
              // Giving the User a Score Bump in case they have already completed CSRF7 and this is
              // the 20th time the unit test has run
              ...
              if (scoreboardData.isEmpty()) {
                ...
                if (scoreBefore == 0) {
                  log.fatal(
                      "Could not find user " + userName + " with score > 0: " + scoreboardData);
                                      ...
                                      // as well incase rounded d
                {
                  ...
                  if (roundedUp != expectedAfter) fail("Invalid Score Deduction Detected");
                  else return; // PASS
                } else {
                  return; // Pass
                }
  ...
  public void testSetModuleCategoryStatusOpen() {
    ...
    if (!Setter.closeAllModules(applicationRoot)) fail("Could not Mark all modules as closed");
  ...
  public void testSetModuleStatusClosed() {
    ...
    if (!Setter.openAllModules(applicationRoot, false)) fail("Could not Mark all modules as open");
  ...
  public void testSuspendUser() {
    ...
    try {
      ...
      try {
        ...
        // Gotta Sleep for a sec otherwise the time setting for suspension will fail test. Must be 1
        // sec after unsuspend function ran
        Thread.sleep(1000);
        loggedIn = GetterTest.verifyTestUser(applicationRoot, userName, userName);
      }
      if (!loggedIn) {
      ...
      if (!GetterTest.verifyTestUser(applicationRoot, userName, userName)) {
        ...
        if (!Setter.suspendUser(applicationRoot, userId, 10)) {
          ...
          if (!Setter.unSuspendUser(applicationRoot, userId)) {
            ...
            // Gotta Sleep for a sec, otherwise the time compair will round down and user auth will
            // fail. User is unsuspended 1 second after unsuspend funciton
            Thread.sleep(1000);
            String user[] = Getter.authUser(applicationRoot, userName, userName);
            if (user == null || user[0].isEmpty()) {
  ...
  public void testUpdatePlayerClass() {
    ...
    try {
      ...
      if (!GetterTest.verifyTestUser(applicationRoot, userName, userName, classId)) {
        ...
        if (!Setter.updatePlayerClass(applicationRoot, newClass, userId)
  ...
  public void testMutipleClassMedals() {
    ...
    try {
      ...
      if (TestProperties.verifyTestUser(log, applicationRoot, userName, userName, classOne)
                ...
                if (scoreboardData.isEmpty()) {
                  ...
                  for (int i = 0; i < scoreboardJson.size(); i++) {
                    ...
                    if (scoreRowJson.get("username").toString().compareTo(userName) == 0) {
                      ...
                      goldMedal =
                          Integer.parseInt(scoreRowJson.get("goldMedalCount").toString()) > 0;
                      break;
                    }
                  }
                  if (!goldMedal) {
                    String message =
                        userName
                            + " should have a gold medal and does not. They were first in their"
                            + " class to complete challenge "
                            + moduleId;
                    log.fatal(message);
                    fail(message);
                  } else {
                    ...
                    for (int i = 0; i < scoreboardJson.size(); i++) {
                      ...
                      if (scoreRowJson.get("username").toString().compareTo(otherUserName) == 0) {
                        log.debug(
                            "Found user with goldMedalCount: "
                                + scoreRowJson.get("goldMedalCount"));
                        goldMedal =
                            Integer.parseInt(scoreRowJson.get("goldMedalCount").toString()) > 0;
                        break;
                      }
